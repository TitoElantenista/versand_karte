...
def generar_mapa(coordenadas_nombres, comentarios, optionen, thumbnail_groesse, centro, uploaded_files, foto_destacada=None):
    ...
    linea = folium.PolyLine(coords_linea, color="blue", weight=2.5, opacity=0.7).add_to(m)


    # Dibujar una pequeña línea después de la primera foto para indicar la dirección
    coords_direccion = primeros_dos_puntos_distantes(coords_linea)
    if len(coords_direccion) == 2:
        linea_direccion = folium.PolyLine(coords_direccion, color="blue", weight=2.5, opacity=0).add_to(m)  # Línea azul


        # Colocar una flecha roja en la línea de dirección
        flecha = PolyLineTextPath(
            linea_direccion,
            '►',
            center=True,
            offset=7,
            attributes={'fill': 'red', 'font-weight': 'bold', 'font-size': '30'}
        ).add_to(m)
